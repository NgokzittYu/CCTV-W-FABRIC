...
flowchart TB
    ...
    subgraph STORAGE["💾 分布式存储层 · MinIO 保存原始证据"]
        direction LR
        MINIO_GOP["视频 GOP 分片存储"]
        MINIO_META["语义 JSON & Merkle 结构"]
        MINIO_CID["SHA-256 内容寻址 (CID)"]

        MINIO_GOP --- MINIO_META
        MINIO_META --- MINIO_CID
    end
    ...
    MK -.->|"GOP 原始数据"| MINIO_GOP
    GW_EPOCH -->|"EpochRoot"| SC
    MINIO_CID -.->|"离线取证"| TRI_STATE
...
在 GOP 级视频存证管线中，为了解决重压缩造成的误报问题，系统引入了**多模态融合视频指纹（Video Integrity Fingerprint, VIF）作为完整性快速分类器**。VIF 通过一小段紧凑的定长指纹（256-bit），在链下快速判断视频状态，避免转码引发的证据失效。系统定义的威胁模型包含三类验证态：
...
- **RE-ENCODED (重压缩/合法转码) ⚠️**：SHA 不匹配，但 $Risk$ 小于阈值 $<0.18$。判定为合法画质降低（如压转），主要语义与内容连续性未变。
- **TAMPERED (恶意篡改) ❌**：$Risk \ge 0.18$。检测到大面积区域涂抹、帧替换、关键目标刻意擦除等破坏视频监控核心证据的修改。
